...
class RogerRDD(store: IStore) {

  case class Line(index: Int, uuid: UUID, line: String, isFirst: Boolean = false)

  def convertLines(index: Int, dataArr: Array[String], isSplit: Boolean,
  ...
  def getLines(directory: IPath): RDD[String] = {
    ...
    }).map(line => ((line.uuid, line.index), line)).reduceByKey((acc, curr) => {
